...
import { createVisaApplication_Test } from '@/lib/firebase/visa-service';
...
export async function POST(req: Request) {
  const { formData }: { formData: TravelerFormData } = await req.json();
    
    try {
      console.log('Creating visa application...');
      const visaId = await createVisaApplication_Test(formData);
